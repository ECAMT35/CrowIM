...
public interface MessagePrivateChatMapper extends BaseMapper<MessagePrivateChat> {
    ...
    @Update("update message_private_chat set status=2 where id=#{messagePrivateChatId} and receiver_id=#{receiverId} and status=1")
    ...
    @Update("update message_private_chat set status=4 where id=#{messagePrivateChatId} and receiver_id=#{receiverId} and status=3")
